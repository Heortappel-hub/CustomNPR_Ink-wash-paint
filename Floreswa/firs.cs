...
public class Ink : MonoBehaviour
{
 ...
 [Range(0.2f, 5.0f)] public float bleedFadeGamma = 1.5f;
    ...
    const int PASS_STIPPLE   = 4;
    const int PASS_COMBINE   = 5;
    const int PASS_COLOR     = 6;
    const int PASS_INK_BLEED = 7;
...
inkMaterial.SetFloat("_BleedDarkSoftness", bleedDarkSoftness);
    ...
    // 反投影矩阵：shader 里用来把屏幕像素重建到世界空间
  Camera cam = GetComponent<Camera>();
    // depth 贴图按 GPU 约定写入（D3D reversed-Z + y 翻转），必须用 GL.GetGPUProjectionMatrix
  // 得到一致的投影矩阵，否则重建的世界坐标会随相机姿态漂移
...
int height = useImage ? image.height : source.height;
 ...
 // --- Luminance ---
  RenderTexture luminanceRT = RenderTexture.GetTemporary(width, height, 0, fmt);
     ...
     // --- Edge Detection（Contrast / Sobel / Prewitt / DoG）---
 ...
 // Bleed Pass 需要原始亮度图做暗部门控
        inkMaterial.SetTexture("_LumTex", luminanceRT);
